...
- UNBREAKABLE REWRITE LAWS:
  ...
  4. PUNCTUATION RULE: Use commas or full stops only. Absolutely no hyphens or dashes used as punctuation.
            ...
            try:
                response = openai.ChatCompletion.create(
                    ...
                    messages=[
                        ...
                        {"role": "user", "content": fwith st.sidebar:
    st.header("RECRUITMENT COMMAND CENTRE")
    api_key = st.text_input("Enter OpenAI API Key", type="password")
    st.info("Your data is processed and then discarded. No waffle. No traces.")
...
if uploaded_file and api_key:
    ...
    if st.button("COMMENCE INSPECTION"):
        with st.spinner("THE MAJOR IS SCREAMING AT YOUR MARGINS..."):
            try:
                ...
                # 7. PDF Download Button (This was missing!)
    ...
    st.warning("I CAN'T WORK WITHOUT AMMUNITION. ENTER YOUR API KEY IN THE SIDEBAR.")with st.sidebar:
    st.header("RECRUITMENT COMMAND CENTRE")
    api_key = st.text_input("Enter OpenAI API Key", type="password")
    st.info("Your data is processed and then discarded. No waffle. No traces.")
